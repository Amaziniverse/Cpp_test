...
#include "MFC1Doc.h"			// 결합을 위해 두 헤더 파일을 가져온다. 
#include "MFC1View.h"
...
void CMFC1View::OnDraw(CDC* pDC)		// 처음 만들고 주석 처리된 이 부분을 푼다! 
{
...
//	pDC->TextOut(0, 0, temp, strlen(temp));			// 현재 버전에서는 동작하지 않는다.
//	pDC->TextOut(0, 0, (LPCWSTR)temp, strlen(temp));		// 이렇게 할 경우 글자가 다 깨졌다.

	CString test = _T("Hello MFC SDI GDI interface test!!");
	pDC->TextOut(0, 0, test);	

	test.Format(_T("HORZRES(가로크기) = %d"), pDC->GetDeviceCaps(HORZRES));
	pDC->TextOut(0, 20, test);
	test.Format(_T("VERTRES(세로크기) = %d"), pDC->GetDeviceCaps(VERTRES));
	pDC->TextOut(0, 40, test);

	test = _T("DrawTest Example");
		// set bod area
		CRect rect = CRect(0, 60, 400, 100);
	// call DrawText function
		pDC->DrawText(test, rect, DT_SINGLELINE | DT_CENTER | DT_VCENTER);
	
	test="ExtTextOut Example";  //foreground color is blue
